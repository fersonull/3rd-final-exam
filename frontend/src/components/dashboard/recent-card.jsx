...
export default function RecentCard({ type = "finished", title, time, className = "", ...props }) {
  const config = typeConfig[type] || typeConfig.finished;

  return (
    <div
      className={`flex items-center gap-3 p-2 rounded-lg border-l-4 ${config.bg} ${config.border} ${className}`} {...props}
    >
      {config.icon}
